...
if [[ $response =~ ^(yes|y) ]]; then
    ...
    printf "       Descriptive name [ttn-rak833]:"
    read GATEWAY_NAME
    if [[ $GATEWAY_NAME == "" ]]; then GATEWAY_NAME="ttn-rak833"; fi
...
cp $SCRIPT_DIR/rak833_rpi.h ./libloragw/inc/rak833_rpi.h
...
sed -i -e 's/PLATFORM= kerlink/PLATFORM= rak833_rpi/g' ./libloragw/library.cfg
